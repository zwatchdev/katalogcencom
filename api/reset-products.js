...
function normalizeProducts(items) {
  ...
  return items
    .map((item) => ({
      ...
      product_type: ["Laptop", "Printer", "Monitor", "Komputer", "Proyektor"].includes(item.productType)
        ? item.productType
        : "Laptop",
      category: ["Gaming", "Office", "Kuliahan"].includes(item.category) ? item.category : "Office",
      price_mode: String(item.priceMode || item.price_mode || "").trim() === "Kredit" ? "Kredit" : "Cash",
      description: String(item.description || "").trim(),
      feature: String(item.feature || "").trim(),
      image_data: String(item.imageData || item.image_data || "").trim(),
