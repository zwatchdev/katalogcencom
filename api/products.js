...
function normalizeProduct(rawProduct) {
  ...
  const hidden = Boolean(product.hidden || product.is_hidden);
  ...
  try {
    ...
    if (req.method === "GET") {
      const { data, error } = await supabase
        .from("products")
        .select("id, name, price, stock, product_type, category, price_mode, is_hidden, description, feature, image_data, specs, created_at, updated_at")
      ...
      res.status(200).json({
        products: (data || []).map((product) => ({
          ...
          hidden: Boolean(product.is_hidden),
