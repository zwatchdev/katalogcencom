...
function saveCreditDashboardExportState() {
  localStorage.setItem(creditExportStateStorageKey, JSON.stringify(creditDashboardExportState));
...
function handleSiteBannerUpload(file) {
  if (!file) {
    return;
  }
  ...
  const reader = new FileReader();
  reader.onload = () => {
    pendingSiteBannerData = String(reader.result);
...
if (siteBannerInput) {
  siteBannerInput.addEventListener("change", (event) => {
    handleSiteBannerUpload(event.target.files[0]);
