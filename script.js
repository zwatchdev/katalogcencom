...
const siteSettingsKey = "central-computer-site-settings";
const exportScope = document.body?.classList.contains("export-credit-page") ? "credit" : "default";
const exportStateKey = exportScope === "credit"
  ? "central-computer-export-state-credit"
  : "central-computer-export-state";
const exportSettingsKey = exportScope === "credit"
  ? "central-computer-export-settings-credit"
  : "central-computer-export-settings";
const creditExportStateStorageKey = "central-computer-export-state-credit";
const creditExportSettingsStorageKey = "central-computer-export-settings-credit";
...
const remoteMediaUploadEndpoint = "/api/media-upload";
...
const productPriceModeInput = document.getElementById("productPriceMode");
...
const creditDashboardList = document.getElementById("creditDashboardList");
const creditDashboardPagination = document.getElementById("creditDashboardPagination");
const creditProductCountBadge = document.getElementById("creditProductCountBadge");
const creditExportSelectionCount = document.getElementById("creditExportSelectionCount");
const selectAllCreditProductsCheckbox = document.getElementById("selectAllCreditProductsCheckbox");
...
const siteBannerInput = document.getElementById("siteBannerInput");
const siteBannerPreview = document.getElementById("siteBannerPreview");
const clearSiteBannerButton = document.getElementById("clearSiteBannerButton");
...
const exportBrochureDateInput = document.getElementById("exportBrochureDate");
...
const catalogProductTabs = [...productTypes, "Kredit"];
...
let creditDashboardCurrentPage = 1;
...
let creditDashboardExportState = loadStoredExportState(creditExportStateStorageKey);
...
function normalizeCatalogTab(value) {
  return catalogProductTabs.includes(value) ? value : "Laptop";
}
...
function cloneProducts(items) {
  return items.map((product) => ({
    ...
    category: ["Gaming", "Office", "Kuliahan"].includes(product.category) ? product.category : "Office",
    priceMode: normalizePriceMode(product.priceMode || (product.category === "Kredit" ? "Kredit" : "Cash")),
    hidden: Boolean(product.hidden || product.is_hidden),
...
function shouldPersistInlineImage(imageData) {
...
function sanitizeProductsForCache(items) {
  return items.map((product) => ({
    ...product,
    imageData: shouldPersistInlineImage(product.imageData) ? String(product.imageData || "") : ""
  }));
}

function sanitizeExportSettingsForCache(settings) {
  const nextSettings = settings && typeof settings === "object" ? settings : {};
  const customImagesByPage = nextSettings.customImagesByPage && typeof nextSettings.customImagesByPage === "object"
    ? Object.fromEntries(
        Object.entries(nextSettings.customImagesByPage).map(([page, images]) => [
          page,
          Array.isArray(images)
            ? images.map((image) => (shouldPersistInlineImage(image) ? String(image || "") : ""))
            : ["", "", ""]
        ])
      )
    : { "1": ["", "", ""] };
  ...
  return {
    ...nextSettings,
    logoImage: shouldPersistInlineImage(nextSettings.logoImage) ? String(nextSettings.logoImage || "") : "",
    customImagesByPage
  };
}

function sanitizeSiteSettingsForCache(settings) {
  const nextSettings = settings && typeof settings === "object" ? settings : {};

  return {
    ...nextSettings,
    bannerImage: shouldPersistInlineImage(nextSettings.bannerImage) ? String(nextSettings.bannerImage || "") : ""
  };
}
...
function saveProducts() {
  safeLocalStorageSetItem(
    storageKey,
    JSON.stringify(sanitizeProductsForCache(products)),
    "Penyimpanan browser penuh. Hapus beberapa gambar produk atau pindahkan data ke mode remote Supabase."
  );
}

function loadStoredExportState(storageKey) {
...
function loadExportState() {
  return loadStoredExportState(exportStateKey);
}
...
function saveExportState(options = {}) {
  safeLocalStorageSetItem(
    ...
    "Penyimpanan browser penuh saat menyimpan pilihan export."
  );
...
function loadStoredExportSettings(storageKey) {
...
function loadExportSettings() {
  return loadStoredExportSettings(exportSettingsKey);
}

function saveExportSettings(options = {}) {
  safeLocalStorageSetItem(
    exportSettingsKey,
    JSON.stringify(sanitizeExportSettingsForCache(exportSettings)),
    "Penyimpanan browser penuh saat menyimpan pengaturan export."
  );
...
function loadSiteSettings() {
  ...
  try {
    ...
    return {
      ...defaultSiteSettings,
      ...parsed,
      bannerImage: String(parsed.bannerImage || "")
    };
  } catch (error) {
    return { ...defaultSiteSettings };
  }
...
function saveSiteSettings(options = {}) {
  safeLocalStorageSetItem(
    siteSettingsKey,
    JSON.stringify(sanitizeSiteSettingsForCache(siteSettings)),
    "Penyimpanan browser penuh saat menyimpan banner katalog."
  );

  if (options.remote !== false) {
    queueRemoteSiteSettingsSave();
  }
}
...
function safeLocalStorageSetItem(key, value, quotaMessage) {
  try {
    localStorage.setItem(key, value);
  } catch (error) {
    if (isQuotaExceededError(error)) {
      throw new Error(quotaMessage || "Penyimpanan browser penuh.");
    }
...
async function uploadImageDataUrlToStorage(dataUrl, folder, fileNameBase) {
  if (!isRemoteDataMode()) {
    throw new Error("Upload gambar ke Supabase Storage hanya tersedia saat mode remote aktif.");
  }

  const payload = await fetchApiJson(remoteMediaUploadEndpoint, {
    method: "POST",
    body: JSON.stringify({
      dataUrl,
      folder,
  ...
  if (!payload.publicUrl) {
    throw new Error("Upload gambar gagal tanpa URL publik.");
  }

  return payload.publicUrl;
}

function buildExportPreferencesEndpoint() {
  if (exportScope === "credit") {
    return `${remoteExportPreferencesEndpoint}?scope=credit`;
  }

  return remoteExportPreferencesEndpoint;
}

function buildScopedExportPreferencesEndpoint(scope) {
  return scope === "credit"
    ? `${remoteExportPreferencesEndpoint}?scope=credit`
    : remoteExportPreferencesEndpoint;
}
...
function normalizeProductRecord(product) {
  const legacyCreditMode = String(product.category || "").trim() === "Kredit";
  ...
  return {
    ...
    category: ["Gaming", "Office", "Kuliahan"].includes(product.category) ? product.category : "Office",
    priceMode: normalizePriceMode(product.priceMode || product.price_mode || (legacyCreditMode ? "Kredit" : "Cash")),
    hidden: Boolean(product.hidden || product.is_hidden),
    description: String(product.description || ""),
    feature: String(product.feature || ""),
    imageData: String(product.imageData || product.image_data || ""),
...
async function loadRemoteSiteSettings() {
  try {
    const payload = await fetchApiJson(remoteSiteSettingsEndpoint, { method: "GET" });
    siteSettings = {
      ...defaultSiteSettings,
      ...(payload.settings || {}),
      bannerImage: String(payload.settings?.bannerImage || "")
    };
    pendingSiteBannerData = siteSettings.bannerImage;
    saveSiteSettings({ remote: false });
...
function normalizeExportSettingsRecord(settings) {
  ...
  return {
    ...
    brochureDate: String(nextSettings.brochureDate || ""),
...
async function loadRemoteExportPreferences() {
  ...
  const payload = await fetchApiJson(buildExportPreferencesEndpoint(), { method: "GET" });
...
function queueRemoteExportPreferencesSave() {
  ...
  exportPreferencesSaveTimer = window.setTimeout(async () => {
    ...
    try {
      await fetchApiJson(buildExportPreferencesEndpoint(), {
        method: "PUT",
        body: JSON.stringify({
...
function queueRemoteSiteSettingsSave() {
  if (!isRemoteDataMode()) {
    return;
  }

  if (siteSettingsSaveTimer) {
    window.clearTimeout(siteSettingsSaveTimer);
  }

  siteSettingsSaveTimer = window.setTimeout(async () => {
    siteSettingsSaveTimer = null;

    try {
      await fetchApiJson(remoteSiteSettingsEndpoint, {
        method: "PUT",
        body: JSON.stringify({ settings: siteSettings })
      });
    } catch (error) {
      // Keep local state as fallback if autosave fails.
    }
  }, 500);
}

function isCreditProduct(product) {
  return normalizePriceMode(product?.priceMode || (product?.category === "Kredit" ? "Kredit" : "Cash")) === "Kredit";
}

function matchesCurrentExportScope(product) {
  if (exportScope === "credit") {
    return isCreditProduct(product);
  }
...
function isCreditDashboardSelected(productId) {
  return creditDashboardExportState.selectedProductIds.includes(productId);
}
...
function saveCreditDashboardExportState() {
  safeLocalStorageSetItem(
    creditExportStateStorageKey,
    JSON.stringify(creditDashboardExportState),
    "Penyimpanan browser penuh saat menyimpan pilihan export kredit."
  );
  ...
  const creditSettings = loadStoredExportSettings(creditExportSettingsStorageKey);
  fetchApiJson(buildScopedExportPreferencesEndpoint("credit"), {
    method: "PUT",
    body: JSON.stringify({
      settings: creditSettings,
      state: creditDashboardExportState
    })
  }).catch(() => {});
}

function toggleCreditDashboardSelection(productId, checked) {
  if (checked) {
    if (!creditDashboardExportState.selectedProductIds.includes(productId)) {
      creditDashboardExportState.selectedProductIds.push(productId);
    }
  } else {
    creditDashboardExportState.selectedProductIds = creditDashboardExportState.selectedProductIds.filter((id) => id !== productId);
  }

  if (!creditDashboardExportState.manualOrder) {
    creditDashboardExportState.selectedProductIds = sortProductIdsByBrandAndName(creditDashboardExportState.selectedProductIds);
  }

  saveCreditDashboardExportState();
}
...
function renderSiteBannerPreview() {
  if (!siteBannerPreview) {
    return;
  }

  if (!pendingSiteBannerData) {
    siteBannerPreview.classList.add("d-none");
    siteBannerPreview.innerHTML = "";
    return;
  }

  siteBannerPreview.classList.remove("d-none");
  siteBannerPreview.innerHTML = `
    <img src="${pendingSiteBannerData}" alt="Preview banner katalog" class="product-image-preview-img">
  `;
}

function renderCatalogBanner() {
  if (!catalogBannerSection || !catalogBannerImage) {
    return;
  }

  if (!siteSettings.bannerImage) {
    catalogBannerSection.classList.add("d-none");
    catalogBannerImage.removeAttribute("src");
    if (catalogBannerButton) {
      catalogBannerButton.disabled = true;
    }
    return;
  }

  catalogBannerImage.src = siteSettings.bannerImage;
  catalogBannerSection.classList.remove("d-none");
  if (catalogBannerButton) {
    catalogBannerButton.disabled = false;
  }
}
...
async function handleProductImageUpload(file) {
  ...
  try {
    ...
    pendingProductImageData = isRemoteDataMode()
      ? await uploadImageDataUrlToStorage(compressedImage, "product-images", "product-image")
      : compressedImage;
    renderProductImagePreview();
  } catch (error) {
    Swal.fire({
...
async function handleSiteBannerUpload(file) {
  if (!file) {
    return;
  }
  ...
  if (file.size > maxBannerImageBytes) {
    Swal.fire({
      ...
      text: "Ukuran maksimal banner adalah 2MB.",
      icon: "warning",
      confirmButtonText: "Tutup",
      confirmButtonColor: "#ca1111"
    });
  ...
  try {
    ...
    pendingSiteBannerData = isRemoteDataMode()
      ? await uploadImageDataUrlToStorage(compressedImage, "site-settings", "catalog-banner")
      : compressedImage;
    siteSettings.bannerImage = pendingSiteBannerData;
    saveSiteSettings();
    renderSiteBannerPreview();
    renderCatalogBanner();
  } catch (error) {
    Swal.fire({
      title: "Gagal memproses banner",
      text: error.message,
      icon: "error",
      confirmButtonText: "Tutup",
      confirmButtonColor: "#ca1111"
...
function isCashCategory(category) {
  return ["Gaming", "Office", "Kuliahan"].includes(String(category || "").trim());
}

function updateCategoryQuickButtons() {
  if (!productForm || !productPriceModeInput) {
    return;
  }

  const isCreditMode = normalizePriceMode(productPriceModeInput.value) === "Kredit";
  productForm.querySelectorAll("[data-product-category-quick]").forEach((button) => {
    const mode = button.dataset.productCategoryQuick;
    const isActive = (mode === "credit" && isCreditMode) || (mode === "cash" && !isCreditMode);
...
function fillProductForm(product, options = {}) {
  ...
  productIdInput.value = options.isDuplicate ? "" : product.id;
  productNameInput.value = String(overrides.name ?? product.name ?? "");
  productPriceInput.value = Number(overrides.price ?? product.price ?? 0);
  productStockInput.value = Number(overrides.stock ?? product.stock ?? 0);
  productTypeInput.value = normalizeProductType(overrides.productType ?? product.productType);
  productCategoryInput.value = String(overrides.category ?? product.category ?? "Office");
  if (productPriceModeInput) {
    productPriceModeInput.value = normalizePriceMode(overrides.priceMode ?? product.priceMode);
  }

  if (productDescriptionInput) {
    productDescriptionInput.value = String(overrides.description ?? product.description ?? "");
  }

  if (productFeatureInput) {
    productFeatureInput.value = String(overrides.feature ?? product.feature ?? "");
  }

  pendingProductImageData = String(overrides.imageData ?? product.imageData ?? "");
  renderProductImagePreview();
  renderSpecFields(Array.isArray(overrides.specs) ? overrides.specs : product.specs);
  updateProductEditorMode();
  updateCategoryQuickButtons();
...
function resetForm() {
  ...
  if (productPriceModeInput) {
    productPriceModeInput.value = "Cash";
  }
  if (productDescriptionInput) {
...
async function duplicateProductToTargetCategory(productId, targetCategory) {
  const product = products.find((item) => item.id === productId);
  ...
  const normalizedTargetPriceMode = normalizePriceMode(targetCategory);
  const duplicatedProduct = {
    ...product,
    id: createId(),
    priceMode: normalizedTargetPriceMode,
    hidden: false,
    category: isCashCategory(product.category) ? product.category : "Office",
    description: String(product.description || ""),
    feature: String(product.feature || ""),
    imageData: String(product.imageData || ""),
    specs: Array.isArray(product.specs)
      ? product.specs.map((spec) => [String(spec?.[0] || ""), String(spec?.[1] || "")])
      : []
  };

  try {
    if (isRemoteDataMode()) {
      await createRemoteProduct(duplicatedProduct);
    ...
    Swal.fire({
      title: normalizedTargetPriceMode === "Kredit" ? "Gagal menduplikat ke kredit" : "Gagal menduplikat ke cash",
      text: error.message,
      icon: "error",
      confirmButtonText: "Tutup",
      confirmButtonColor: "#ca1111"
  ...
  Swal.fire({
    title: normalizedTargetPriceMode === "Kredit" ? "Produk berhasil diduplikat ke kredit" : "Produk berhasil diduplikat ke cash",
    text: normalizedTargetPriceMode === "Kredit"
      ? "Produk baru sudah masuk ke daftar Data Produk Kredit."
      : "Produk baru sudah masuk ke daftar Data Produk Cash.",
    icon: "success",
    confirmButtonText: "Tutup",
    confirmButtonColor: "#ca1111"
  });
...
async function toggleProductHidden(productId) {
  ...
  const nextHiddenState = !Boolean(product.hidden);
  const nextProduct = {
    ...product,
  ...
  try {
    ...
    Swal.fire({
      title: nextHiddenState ? "Gagal menyembunyikan produk" : "Gagal menampilkan produk",
      text: error.message,
      icon: "error",
      confirmButtonText: "Tutup",
      confirmButtonColor: "#ca1111"
  ...
  Swal.fire({
    title: nextHiddenState ? "Produk disembunyikan" : "Produk ditampilkan lagi",
    text: nextHiddenState
      ? "Produk tidak akan muncul di katalog publik."
      : "Produk kembali muncul di katalog publik.",
    icon: "success",
    confirmButtonText: "Tutup",
    confirmButtonColor: "#ca1111"
  });
...
function renderDashboardList() {
  ...
  const cashSelectedIds = exportState.selectedProductIds.filter((productId) => {
    ...
    return product && !isCreditProduct(product);
  });
  creditDashboardExportState.selectedProductIds = creditDashboardExportState.selectedProductIds.filter((productId) => {
    const product = findProduct(productId);
    return product && isCreditProduct(product);
  });
  const creditSelectedIds = [...creditDashboardExportState.selectedProductIds];

  productCountBadge.textContent = `${filteredDashboardProducts.length} Produk`;
  if (creditProductCountBadge) {
    creditProductCountBadge.textContent = `${filteredCreditProducts.length} Produk Kredit`;
  }
  if (creditExportSelectionCount) {
    creditExportSelectionCount.textContent = `${creditSelectedIds.length} Dipilih`;
  }
  if (exportSelectionCount) {
    exportSelectionCount.textContent = `${cashSelectedIds.length} Dipilih`;
  }
  if (selectAllProductsCheckbox) {
    const cashProducts = products.filter((product) => !isCreditProduct(product));
    const allSelected = cashProducts.length > 0 && cashSelectedIds.length === cashProducts.length;
    selectAllProductsCheckbox.checked = allSelected;
    selectAllProductsCheckbox.indeterminate = !allSelected && cashSelectedIds.length > 0;
  }
  if (selectAllCreditProductsCheckbox) {
    const creditProducts = products.filter((product) => isCreditProduct(product));
    const allCreditSelected = creditProducts.length > 0 && creditSelectedIds.length === creditProducts.length;
    selectAllCreditProductsCheckbox.checked = allCreditSelected;
    selectAllCreditProductsCheckbox.indeterminate = !allCreditSelected && creditSelectedIds.length > 0;
  }

  renderDashboardSection({
    items: filteredDashboardProducts,
    listElement: dashboardList,
    paginationElement: dashboardPagination,
    countElement: productCountBadge,
    currentPage: dashboardCurrentPage,
    pageDataAttribute: "data-dashboard-page",
    emptyTitle: "Produk cash tidak ditemukan",
    emptyDescription: "Coba kata kunci lain untuk nama, stok, atau spesifikasi produk cash.",
    includeSelection: true
  });

  if (creditDashboardList && creditDashboardPagination) {
    renderDashboardSection({
      items: filteredCreditProducts,
      listElement: creditDashboardList,
      paginationElement: creditDashboardPagination,
      countElement: creditProductCountBadge,
      currentPage: creditDashboardCurrentPage,
      pageDataAttribute: "data-credit-dashboard-page",
      emptyTitle: "Produk kredit tidak ditemukan",
      emptyDescription: "Tambahkan produk mode Kredit atau gunakan kata kunci lain.",
      includeSelection: true,
      selectionScope: "credit"
    });
  }
}
...
function renderDashboardSection({
  ...
  if (!items.length) {
    listElement.innerHTML = `
      <div class="dashboard-empty">
        ...
        <p class="mb-0">${escapeHtml(emptyDescription)}</p>
      </div>
    `;
  ...
  const totalPages = Math.ceil(items.length / dashboardItemsPerPage);
  const safeCurrentPage = Math.min(currentPage, totalPages);

  if (pageDataAttribute === "data-dashboard-page") {
    dashboardCurrentPage = safeCurrentPage;
  } else if (pageDataAttribute === "data-credit-dashboard-page") {
    creditDashboardCurrentPage = safeCurrentPage;
  }

  const startIndex = (safeCurrentPage - 1) * dashboardItemsPerPage;
  const visibleItems = items.slice(startIndex, startIndex + dashboardItemsPerPage);

  listElement.innerHTML = visibleItems
    .map((product) => createDashboardItemMarkup(product, includeSelection, selectionScope))
    .join("");

  renderDashboardPagination(totalPages, safeCurrentPage, paginationElement, pageDataAttribute);
}

function createDashboardItemMarkup(product, includeSelection, selectionScope = "cash") {
  const duplicateTargetCategory = isCreditProduct(product) ? "Cash" : "Kredit";
  const priceModeLabel = isCreditProduct(product) ? "Kredit" : "Cash";
  const duplicateButtonLabel = isCreditProduct(product) ? "Duplikat ke Cash" : "Duplikat ke Kredit";
  const hideButtonLabel = product.hidden ? "Tampilkan Produk" : "Hide Produk";
  const isSelected = selectionScope === "credit"
    ? isCreditDashboardSelected(product.id)
    : isProductSelected(product.id);
  const selectionLabel = selectionScope === "credit" ? "Pilih Export Kredit" : "Pilih Export";
  ...
  return `
    <article class="dashboard-item">
      <div>
        ...
        <p class="dashboard-item-meta mb-0">Mode ${escapeHtml(priceModeLabel)} | Jenis ${escapeHtml(normalizeProductType(product.productType))} | Kategori ${escapeHtml(product.category)} | Stok ${product.stock} | ${
          usesDescriptionField(product.productType)
            ? "Deskripsi produk"
            : `${product.specs.length} spesifikasi`
        }${product.hidden ? " | Status Hidden" : ""}</p>
      </div>
      <div class="dashboard-item-actions">
        ${
          includeSelection
            ...
            `
            : '<span class="dashboard-text">Masuk ke Export</span>'
        }
        <button class="btn btn-danger dashboard-action-button" type="button" data-edit-id="${product.id}">
          Edit
        </button>
        <button
          class="btn btn-outline-danger dashboard-action-button"
          type="button"
          data-duplicate-id="${product.id}"
          data-duplicate-category="${duplicateTargetCategory}"
        >
          ${duplicateButtonLabel}
        </button>
        <button
          class="btn btn-outline-dark dashboard-action-button"
          type="button"
          data-toggle-hidden-id="${product.id}"
        >
          ${hideButtonLabel}
        </button>
...
function renderDashboardPagination(totalPages, currentPage, paginationElement, pageDataAttribute) {
  ...
  paginationElement.innerHTML = `
    <div class="pagination-controls">
      <button
        ...
        type="button"
        ${pageDataAttribute}="${currentPage - 1}"
  ...
  productList.innerHTML = visibleItems
    .map(
      (product) => {
        ...
        const pricingModeBadge = isCreditProduct(product)
          ? '<span class="category-badge category-badge-credit">Kredit</span>'
          : "";
...
function syncCatalog() {
  ...
  filteredProducts = products.filter((product) => {
    ...
    const matchesTab = activeCatalogProductType === "Kredit"
      ? isCreditProduct(product)
      : normalizedProductType === activeCatalogProductType && !isCreditProduct(product);
...
function updateExportSettingsFromForm() {
  ...
  exportSettings.brochureDate = exportBrochureDateInput?.value || "";
...
function fillExportSettingsForm() {
  ...
  if (exportBrochureDateInput) {
    exportBrochureDateInput.value = exportSettings.brochureDate || "";
  }
...
function formatBrochureDate(value) {
...
function renderExportPreview() {
  ...
  if (exportScope === "credit") {
    exportSelectionSummary.textContent = `${selectedProducts.length} produk kredit dipilih untuk export PDF.`;
  }
    ...
    exportPreview.innerHTML = `
      <article class="dashboard-card export-empty-state">
        ...
        <p class="dashboard-text mb-0">
          ${exportScope === "credit"
            ? "Kembali ke dashboard admin, buat produk mode Kredit, lalu cetak dari halaman export PDF kredit."
      ...
      (page, pageIndex) => `
        ...
        >
          <div class="export-page-number">
            ...
            ${
              exportSettings.brochureDate
                ? `<strong class="export-brochure-date">Brosur ${escapeHtml(formatBrochureDate(exportSettings.brochureDate))}</strong>`
                : ""
            }
          </div>
          ${(() => {
...
async function handleCustomImageUpload(index, file) {
  ...
  try {
    ...
    images[index] = isRemoteDataMode()
      ? await uploadImageDataUrlToStorage(dataUrl, "export-images", `export-image-${index + 1}`)
      : String(dataUrl);
    saveExportSettings();
    renderExportPreview();
  } catch (error) {
...
async function handleExportLogoUpload(file) {
  if (!file) {
    return;
  }
  ...
  try {
    ...
    exportSettings.logoImage = isRemoteDataMode()
      ? await uploadImageDataUrlToStorage(compressedImage, "export-images", "export-logo")
      : compressedImage;
    saveExportSettings();
    renderExportPreview();
  } catch (error) {
...
window.showBannerPreview = function showBannerPreview() {
  if (!siteSettings.bannerImage) {
    return;
  }

  Swal.fire({
    title: "Preview Banner Katalog",
    html: `
      <div class="banner-preview-toolbar">
        <button type="button" class="btn btn-outline-dark" data-banner-zoom="out">-</button>
        <button type="button" class="btn btn-outline-danger" data-banner-zoom="reset">Reset</button>
        <button type="button" class="btn btn-outline-dark" data-banner-zoom="in">+</button>
      </div>
      <div class="banner-preview-swal-wrap" data-banner-preview-wrap>
        <img src="${siteSettings.bannerImage}" alt="Preview banner katalog" class="banner-preview-swal-image" data-banner-preview-image>
      </div>
    `,
    ...
    didOpen: () => {
      ...
      const previewWrap = popup.querySelector("[data-banner-preview-wrap]");
      const previewImage = popup.querySelector("[data-banner-preview-image]");
      const zoomButtons = popup.querySelectorAll("[data-banner-zoom]");
      let zoomLevel = 1;
      ...
      const setZoomLevel = (nextZoomLevel) => {
        zoomLevel = Math.min(3, Math.max(0.5, nextZoomLevel));
        applyZoom();
      };

      zoomButtons.forEach((button) => {
        button.addEventListener("click", () => {
          const mode = button.dataset.bannerZoom;
      ...
      previewWrap.addEventListener("wheel", (event) => {
        ...
        setZoomLevel(zoomLevel + (event.deltaY < 0 ? 0.15 : -0.15));
      }, { passive: false });
...
if (productForm) {
  productForm.addEventListener("click", (event) => {
    const quickButton = event.target.closest("[data-product-category-quick]");

    if (!quickButton || !productCategoryInput) {
      return;
    }

    const mode = quickButton.dataset.productCategoryQuick;

    if (mode === "credit") {
      if (productPriceModeInput) {
        productPriceModeInput.value = "Kredit";
      }
    } else if (productPriceModeInput) {
      productPriceModeInput.value = "Cash";
    }
...
if (productImageInput) {
  productImageInput.addEventListener("change", (event) => {
    void handleProductImageUpload(event.target.files[0]);
    productImageInput.value = "";
  });
}

if (siteBannerInput) {
  siteBannerInput.addEventListener("change", (event) => {
    void handleSiteBannerUpload(event.target.files[0]);
    siteBannerInput.value = "";
  });
}

if (catalogBannerButton) {
  catalogBannerButton.addEventListener("click", () => {
    showBannerPreview();
  });
}

if (clearSiteBannerButton) {
  clearSiteBannerButton.addEventListener("click", () => {
    pendingSiteBannerData = "";
...
if (productForm) {
  productForm.addEventListener("submit", async (event) => {
    ...
    const priceMode = normalizePriceMode(productPriceModeInput?.value);
    ...
    const nextProduct = {
      ...
      priceMode,
      hidden: productId
        ? Boolean(products.find((product) => product.id === productId)?.hidden)
        : false,
      description,
      feature,
      imageData,
...
if (dashboardList) {
  dashboardList.addEventListener("click", (event) => {
    ...
    const toggleHiddenButton = event.target.closest("[data-toggle-hidden-id]");
    ...
    if (duplicateButton) {
      duplicateProductToTargetCategory(
        duplicateButton.dataset.duplicateId,
        duplicateButton.dataset.duplicateCategory
      );
    }

    if (toggleHiddenButton) {
      toggleProductHidden(toggleHiddenButton.dataset.toggleHiddenId);
    }
...
if (creditDashboardList) {
  creditDashboardList.addEventListener("click", (event) => {
    const editButton = event.target.closest("[data-edit-id]");
    const duplicateButton = event.target.closest("[data-duplicate-id]");
    const toggleHiddenButton = event.target.closest("[data-toggle-hidden-id]");
    const deleteButton = event.target.closest("[data-delete-id]");
  ...
  creditDashboardList.addEventListener("change", (event) => {
    const checkbox = event.target.closest("[data-select-id]");
    ...
    toggleCreditDashboardSelection(checkbox.dataset.selectId, checkbox.checked);
    renderDashboardList();
  });
}
...
if (creditDashboardPagination) {
  creditDashboardPagination.addEventListener("click", (event) => {
    const button = event.target.closest("[data-credit-dashboard-page]");

    if (!button || button.disabled) {
      return;
    }

    creditDashboardCurrentPage = Number(button.dataset.creditDashboardPage);
    renderDashboardList();
  });
}
...
if (selectFilteredCreditButton) {
  selectFilteredCreditButton.addEventListener("click", () => {
    const nextIds = new Set(creditDashboardExportState.selectedProductIds);

    getFilteredDashboardProducts("credit").forEach((product) => {
      nextIds.add(product.id);
    });

    creditDashboardExportState.selectedProductIds = sortProductIdsByBrandAndName(Array.from(nextIds));
    creditDashboardExportState.manualOrder = false;
    saveCreditDashboardExportState();
    renderDashboardList();
  });
}

if (clearCreditSelectionButton) {
  clearCreditSelectionButton.addEventListener("click", () => {
    creditDashboardExportState.selectedProductIds = [];
    creditDashboardExportState.manualOrder = false;
    saveCreditDashboardExportState();
    renderDashboardList();
  });
...
if (selectAllCreditProductsCheckbox) {
  selectAllCreditProductsCheckbox.addEventListener("change", (event) => {
    creditDashboardExportState.selectedProductIds = event.target.checked
      ? sortProductIdsByBrandAndName(
          products
            .filter((product) => isCreditProduct(product))
            .map((product) => product.id)
        )
      : [];
    creditDashboardExportState.manualOrder = false;
...
if (exportSettingsForm) {
  exportSettingsForm.addEventListener("input", (event) => {
    ...
    if (
      target === exportPageSelect ||
      target.matches("[data-custom-image-input]") ||
      target.matches("[data-export-logo-input]")
    ) {
      return;
    }
  ...
  exportSettingsForm.addEventListener("change", (event) => {
    ...
    if (logoInput) {
      void handleExportLogoUpload(logoInput.files[0]);
      logoInput.value = "";
      return;
    }
    ...
    void handleCustomImageUpload(imageIndex, imageInput.files[0]);
    imageInput.value = "";
  });
...
if (catalogCategoryStrip) {
  catalogCategoryStrip.addEventListener("click", (event) => {
    ...
    if (button.dataset.productType === "Kredit") {
      activeCatalogProductType = "Kredit";
    }
