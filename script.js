...
function paginateExportProducts(items) {
  ...
  for (let index = 0; index < items.length; index += exportItemsPerPage) {
    pages.push(items.slice(index, index + exportItemsPerPage));
  }
...
function startEditProduct(productId) {
  if (!productForm || !productIdInput || !productNameInput || !productPriceInput || !productStockInput || !productCategoryInput) {
    return;
  }
      ...
      (product) => `
        <article class="dashboard-item">
          <div>
            ...
            <p class="dashboard-item-meta mb-0">Kategori ${escapeHtml(product.category)} | Stok ${product.stock} | ${product.specs.length} spesifikasi</p>
          </div>
...
function createExportCard(product) {
  const brandTheme = getBrandTheme(product);
  const categoryTheme = getCategoryTheme(product.category);
  ...
  return `
    ...
    >
      <div class="export-card-top">
        <div class="export-card-logo ${exportSettings.logoImage ? "has-image" : "is-empty"}">
          ${exportSettings.logoImage ? `<img src="${exportSettings.logoImage}" alt="Logo toko" class="export-card-logo-image">` : ""}
        </div>
        <div class="export-card-store">CENTRAL COMPUTER SINGKAWANG</div>
      </div>
...
function createCustomImageSlot(imageSrc, index) {
  if (imageSrc) {
    return `
      <div class="export-custom-box">
        <img src="${imageSrc}" alt="Gambar Custom ${index + 1}" class="export-custom-image">
      </div>
    `;
  }
  return "";
...
function renderExportPreview() {
  ...
  exportPreview.innerHTML = pages
    .map(
      (pageItems, pageIndex) => `
        <section
          class="export-sheet ${pageIndex >= (exportPreviewPage - 1) * 3 && pageIndex < exportPreviewPage * 3 ? "" : "export-sheet-hidden"}"
        ...
        >
          ...
          <header class="export-sheet-header">
            <div class="export-sheet-brand">
              <div class="export-sheet-logo ${exportSettings.logoImage ? "has-image" : "is-empty"}">
                ${exportSettings.logoImage ? `<img src="${exportSettings.logoImage}" alt="Logo toko" class="export-sheet-logo-image">` : ""}
              </div>
          ...
          <div class="export-sheet-grid">
            ${pageItems.map((product) => createExportCard(product)).join("")}
          </div>
        </section>
      `
    )
  ...
  productForm.addEventListener("submit", async (event) => {
    ...
    if (
      ...
      Swal.fire({
        ...
        text: "Isi nama produk, harga, stok, kategori, dan minimal satu spesifikasi yang valid.",
        icon: "warning",
  ...
  logoutButton.addEventListener("click", async () => {
    ...
    if (isRemoteDataMode()) {
      try {
        ...
        // ignore logout network error and clear local view state
      }
    }
...
async function bootstrapApplication() {
  ...
  if (loginSection && dashboardSection) {
    showDashboardState();
  } else if (exportGuard && exportWorkspace) {
    showExportPageState();
  } else {
    syncCatalog();
  }
