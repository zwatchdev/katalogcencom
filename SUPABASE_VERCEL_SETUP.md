...
- menyimpan gambar produk dan banner ke Supabase Storage
...
Schema ini juga mencoba membuat bucket Storage `product-images` dengan akses publik, supaya URL gambar bisa langsung dipakai di katalog.
...
- Bucket Storage `product-images` harus tersedia. File [supabase/schema.sql](./supabase/schema.sql) sudah menyiapkannya.
...
Upload gambar sekarang tidak lagi disimpan sebagai `base64` mentah di browser. File dikompresi dulu, lalu diunggah ke Supabase Storage dan hasilnya disimpan sebagai URL publik di produk maupun pengaturan export.
