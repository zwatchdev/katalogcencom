...
create table if not exists public.products (
  ...
  price_mode text not null default 'Cash' check (price_mode in ('Cash', 'Kredit')),
...
alter table public.products
add column if not exists price_mode text not null default 'Cash';
...
check (category in ('Gaming', 'Office', 'Kuliahan'));

alter table public.products drop constraint if exists products_price_mode_check;
alter table public.products
add constraint products_price_mode_check
check (price_mode in ('Cash', 'Kredit'));

update public.products
set
  price_mode = case when category = 'Kredit' then 'Kredit' else coalesce(price_mode, 'Cash') end,
  category = case when category = 'Kredit' then 'Office' else category end
where category = 'Kredit' or price_mode is null;
